...
def test_video_parts_uses_file_uri_not_base64(tmp_path: Path):
    ...
    assert file_val.startswith("file://"), file_val
    assert "base64://" not in file_val
    assert str(video.resolve()) in file_val
...
def test_max_bytes_matches_llbot_raw_file_ceiling():
