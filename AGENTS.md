...
- 磁盘持久化由 `shared/logging/file_sink.py` 的 `install_file_log_sink()` 注册 file sink：每次启动写入 `{stem}.{timestamp}.log`，启动时清理超过 `LOG_FILE_RETENTION`（默认 7 天）的旧文件；与 Web 环形缓冲独立，级别由 `LOG_FILE_LEVEL` 控制。
