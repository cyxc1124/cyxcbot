...
发版必须同步 Helm 版本：仓内 `deploy/helm` 的 `appVersion` / `image.tag`，以及仓外现网 chart `../helm-chart/cyxcbot-chart`（相对本仓根目录）的 `Chart.yaml` `appVersion` 与 `values.yaml` `image.tag`（与发行 tag 同一号，如 `v2.9.0`）。现网 overlay 含拉取密钥，不要把密钥拷回本仓。模板改动先改仓内 `deploy/helm`，再同步到现网（不含密钥）。
...
| 前端页面 | `web/src/pages/` |
| Helm（仓内） | `deploy/helm/` |
| Helm（现网，发版改版本） | `../helm-chart/cyxcbot-chart`（相对本仓根，勿提交密钥） |
