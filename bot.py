...
def _infer_alembic_revision(cur) -> str:
    ...
    if _sqlite_table_exists(cur, "shared_db_groupspecialtitleusage"):
        return "g7h8i9j0k1l2"
    if _sqlite_table_exists(cur, "shared_db_dynamictargetuser") and not _sqlite_table_exists(
        cur, "shared_db_auditlog"
    ):
        return "e5f6a7b8c9d0"
...
def _repair_alembic_version_if_needed(url: str) -> None:
    ...
    try:
        ...
        if not _sqlite_table_exists(cur, "shared_db_user"):
            return
        inferred = _infer_alembic_revision(cur)
        current = cur.execute(
...
_repair_alembic_version_if_needed(_db_url)
