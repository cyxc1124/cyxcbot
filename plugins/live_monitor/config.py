...
class Config(BaseModel):
    ...
    live_monitor_mapping: Dict[str, List[str]] = Field(
        default_factory=dict, description="房间号-群组ID映射配置"
    )

    live_monitor_user_mapping: Dict[str, List[str]] = Field(
        default_factory=dict, description="房间号-好友QQ号映射配置"
    )

    live_at_all: Dict[str, bool] = Field(
        default_factory=dict, description="房间号-是否@全体成员"
    )

    monitor_interval: int = Field(default=60, description="直播状态检查间隔（秒）")

    include_room_info: bool = Field(
        default=True, description="是否在通知中包含房间详细信息"
    )

    bilibili_cookie: Optional[str] = Field(
        default=None, description="B站用户Cookie（可选）"
    )

    use_websocket: bool = Field(default=True, description="是否启用 WebSocket 实时监控")

    message_templates: LiveMessageTemplates = Field(
        default_factory=LiveMessageTemplates, description="直播推送消息模板"
    )
