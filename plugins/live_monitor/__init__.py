...
from .live_monitor import start_live_monitor, stop_live_monitor, live_monitor_instance
...
# 查询直播状态命令
live_status_cmd = on_command("直播状态", aliases={"查直播", "live"}, priority=10, block=True)
...
async def handle_live_status(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    try:
        ...
        if result:
            ...
            await live_status_cmd.finish(f"无法获取房间 {room_id} 的信息，请检查房间号是否正确")
...
# 列出监控房间命令
list_monitor_cmd = on_command("监控列表", aliases={"直播监控列表"}, priority=10, block=True)
