"""Extract searchable text and X URLs from QQ messages."""
...
def _urls_from_payload(payload: object) -> list[str]:
    ...
    def walk(value: object) -> None:
        if isinstance(value, str):
            found.extend(extract_x_urls(value))
            for url in _URL_IN_TEXT.findall(value):
                found.extend(extract_x_urls(url))
    ...
    for segment in event.message:
        if segment.type == "json":
            ...
            if payload is not None:
                ...
                parts.extend(extract_x_urls(segment_data))
        elif segment.type == "xml":
            raw = str(segment.data.get("data", ""))
            parts.extend(extract_x_urls(raw))
