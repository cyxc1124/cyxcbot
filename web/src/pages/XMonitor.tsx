...
import { getXMonitorStatus, triggerXCheck } from '../api/client'
...
export function XMonitorPage() {
  ...
  const loadStatus = useCallback(async () => {
    const status = await getXMonitorStatus()
  ...
  const handleCheck = async () => {
    setChecking(true)
    try {
      const result = await triggerXCheck()
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start gap-4">
        <div className="shrink-0 basis-full md:basis-auto md:max-w-56">
          <h2 className="text-2xl font-bold text-foreground">X 订阅管理</h2>
          <p className="mt-1 text-sm text-muted-foreground">管理 X 博主推文推送到 QQ 群的订阅</p>
        </div>

        {pollSchedule ? (
          <div className="w-fit max-w-full shrink-0 self-start basis-full md:basis-auto">
            <MonitorPollScheduleCard title="当前 API 请求频率（X）" schedule={pollSchedule} />
          </div>
        ) : null}
      ...
      <TargetMappingSection type="x" onTargetsChanged={loadStatus} />
    </div>
  )
}
